...
// The ManifestBlobFunc type is an adapter to allow the use of ordinary
// function as ManifestBlob mutator.
type ManifestBlobFunc func(context.Context, *ent.ManifestBlobMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ManifestBlobFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ManifestBlobMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ManifestBlobMutation", m)
}
