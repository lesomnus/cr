...
func (Unscoped) ManifestBlobScope(_ context.Context) (predicate.ManifestBlob, error) {
	return nil, nil
}
...
func (ss Scopes) ManifestBlobScope(ctx context.Context) (predicate.ManifestBlob, error) {
	ps := make([]predicate.ManifestBlob, 0, len(ss))
	for _, s := range ss {
		p, err := s.ManifestBlobScope(ctx)
		if err != nil {
			return nil, err
		}
		if p == nil {
...
func (ss Scopes) TagScope(ctx context.Context) (predicate.Tag, error) {
	ps := make([]predicate.Tag, 0, len(ss))
	for _, s := range ss {
		p, err := s.TagScope(ctx)
		if err != nil {
			return nil, err
		}
		if p == nil {
