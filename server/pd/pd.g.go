...
// ManifestScope: declared `global`, so it is not behind the wall at all.
func (wall) ManifestScope(ctx context.Context) (predicate.Manifest, error) {
	return nil, nil
}

// ManifestBlobScope: declared `global`, so it is not behind the wall at all.
func (wall) ManifestBlobScope(ctx context.Context) (predicate.ManifestBlob, error) {
	return nil, nil
}
...
// RepositoryScope: declared `global`, so it is not behind the wall at all.
func (wall) RepositoryScope(ctx context.Context) (predicate.Repository, error) {
	return nil, nil
}
...
func (s Sink) Manifest() api.ManifestServiceServer {
	return sinkManifest{s.Server.Manifest(), s.Server.Store, s.w, s.namer, s.joined}
}

// orderManifest is how Manifests come back.
//
// The last column is the key, and it is not decoration: a cursor cannot
// tell apart two rows equal in every column of the order, so the page after
// the first of them either repeats the second or skips it. Rows written by
// one request are stamped a moment apart at best.
var orderManifest = []sqlpage.Order{
	{Column: manifest.FieldDateCreated, Desc: false},
	{Column: manifest.FieldId, Desc: false},
}

const (
	// ManifestPageSize is what a request that did not say gets, and
	// ManifestPageLimit is the most it gets however loudly it asks.
	ManifestPageSize  = 20
	ManifestPageLimit = 100

	// ManifestFilterLimit is how many filters one request may carry. Each is a
	// predicate in the same query, so it is what says how much of the
	// database a request may ask to read -- and it is refused rather than
	// clamped, because dropping half the filters would answer a question
	// nobody asked.
	ManifestFilterLimit = 32
)

// List answers with the Manifests that match any of the given filters, or with
// every one there is if the request named none, a page at a time.
func (s sinkManifest) List(ctx context.Context, req *api.ManifestListRequest) (*api.ManifestListResponse, error) {
	q := s.store.Db.Manifest.Query()

	// Through the same narrowing every generated read goes through, and not
	// by asking the scope alone: what narrows a read is the wall today and
	// the wall and something else tomorrow, and a list that reached past it
	// would be the one read that missed the something else.
	if p, err := bare.ManifestNarrow(ctx, s.store.Scope, nil); err != nil {
		return nil, err
	} else if p != nil {
		q.Where(p)
	}
	...
	if fs := req.GetFilters(); len(fs) > 0 {
		if len(fs) > ManifestFilterLimit {
			return nil, status.Errorf(codes.InvalidArgument,
				"filters: %d of them, and %d is the most one list carries", len(fs), ManifestFilterLimit)
		}

		ps := make([]predicate.Manifest, 0, len(fs))
		for i, f := range fs {
			p, err := filterManifest(f)
			if err != nil {
				return nil, status.Errorf(codes.InvalidArgument, "filters[%d]: %s", i, err)
			}
	...
	if v := req.GetAfter(); v != "" {
		...
		p, err := sqlpage.After(orderManifest, []any{at0, at1})
		if err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "after: %s", err)
		}
	...
	// there -- so a full last page answers with no cursor rather than sending
	// the caller back for an empty one.
	size := sqlpage.Size(int(req.GetSize()), ManifestPageSize, ManifestPageLimit)
	us, err := q.Order(manifest.ByDateCreated(), manifest.ById()).Limit(size + 1).All(ctx)
	if err != nil {
		return nil, err
	}
	...
	items := make([]*api.Manifest, len(us))
	for i, u := range us {
		items[i] = u.Proto()
	}

	res := api.ManifestListResponse_builder{Items: items}.Build()
	if more {
		last := us[len(us)-1]
		next, err := sqlpage.Encode(last.DateCreated, last.Id)
		if err != nil {
...
// filterManifest turns one filter into the predicate that selects what it
// names. Naming nothing is refused, since the request asked for "these" and
// did not say which.
func filterManifest(f *api.ManifestFilter) (predicate.Manifest, error) {
	ps := make([]predicate.Manifest, 0, 1)
	if f.HasRef() {
		p, err := bare.ManifestPick(f.GetRef())
		if err != nil {
			return nil, err
		}
	...
	if f.HasRepo() {
		ps = append(ps, manifest.RepoEQ(f.GetRepo()))
	}
	if f.HasDigest() {
		ps = append(ps, manifest.DigestEQ(f.GetDigest()))
	}
	if f.HasSubject() {
		ps = append(ps, manifest.SubjectEQ(f.GetSubject()))
	}
	if len(ps) == 0 {
		return nil, status.Error(codes.InvalidArgument, "a filter that names nothing")
	}
...
// ManifestService is the prefix of every Rpc of that service, which is how a
// change is known to be about a Manifest. A service is named for the entity it
// is about, so the name carries it.
var ManifestService = watch.ServiceOf(api.ManifestService_Get_FullMethodName)

// Watch answers with the Manifests this caller may see, as they are now and as
// they change.
//
// What is sent is **state and never a delta**, which is what makes a stream
// that missed something still correct: the next item about a row carries the
// whole of it, so a client converges rather than replays. It is also what
// makes the first message safe to duplicate against the ones after it.
func (s sinkManifest) Watch(req *api.ManifestWatchRequest, out grpc.ServerStreamingServer[api.ManifestWatchResponse]) error {
	ctx := out.Context()
	...
	case len(fs) == 0:
		return status.Error(codes.InvalidArgument,
			"filters: a watch says which rows it is about; one that says nothing is the whole table, for as long as it is open")
	case len(fs) > ManifestFilterLimit:
		return status.Errorf(codes.InvalidArgument,
			"filters: %d of them, and %d is the most one watch carries", len(fs), ManifestFilterLimit)
	}

	// Resolved before anything is subscribed to, so a name that names
	// nothing is an answer rather than a stream that quietly watches none.
	watching, err := s.watchManifestKeys(ctx, fs)
	if err != nil {
		return err
	}
	...
	return watch.Stream(ctx, s.w, ManifestService, snapshot,
		func(ks map[pdid.Id]string, sent watch.Seen) error {
			items := make([]*api.ManifestWatchItem, 0, len(ks))
			for k, action := range ks {
				u, err := s.watchRead(ctx, watching, k)
				if err != nil {
					return err
				...
				sent[k] = u != nil
				items = append(items, api.ManifestWatchItem_builder{
					Id:     k.Bytes(),
					Value:  u,
					Action: action,
				}.Build())
			...
			return out.Send(api.ManifestWatchResponse_builder{Items: items}.Build())
		})
}
...
// would have called -- so what a stream begins with and what a list answers
// cannot disagree, and a client does not have to do both and race them.
func (s sinkManifest) watchNow(
	ctx context.Context, req *api.ManifestWatchRequest, out grpc.ServerStreamingServer[api.ManifestWatchResponse],
	sent watch.Seen,
) error {
	after := ""
	for {
		res, err := s.List(ctx, api.ManifestListRequest_builder{
			Filters: req.GetFilters(),
			After:   after,
		}.Build())
		if err != nil {
		...
		items := make([]*api.ManifestWatchItem, 0, len(res.GetItems()))
		for _, u := range res.GetItems() {
			k, err := pdid.From(u.GetId())
			if err != nil {
				return err
			...
			// No action: this is not something anybody asked for, it is
			// what is already there.
			items = append(items, api.ManifestWatchItem_builder{Id: u.GetId(), Value: u}.Build())
		}
		if len(items) > 0 {
			if err := out.Send(api.ManifestWatchResponse_builder{Items: items}.Build()); err != nil {
				return err
			}
		}
...
func (s sinkManifest) watchRead(
	...
	v, err := s.Get(ctx, api.ManifestGetRequest_builder{
		Ref: api.ManifestRef_builder{Id: k.Bytes()}.Build(),
	}.Build())
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return nil, nil
...
// watchManifestKeys is the rows a stream is about, resolved once when it opens.
//
// A filter names a row and a row is named several ways -- by identifier, or
// by whatever unique index the schema declared. Resolving them here rather
// than comparing them per event does three things: the comparison afterwards
...
// row renamed while the stream is open goes on being the row that was asked
// for -- which is what somebody watching a thing meant.
func (s sinkManifest) watchManifestKeys(
	ctx context.Context, fs []*api.ManifestFilter,
) ([]pdid.Id, error) {
	ks := make([]pdid.Id, 0, len(fs))
	for i, f := range fs {
		if !f.HasRef() {
		...
		v, err := s.Get(ctx, api.ManifestGetRequest_builder{Ref: f.GetRef()}.Build())
		if err != nil {
			return nil, err
		}
...
type sinkManifestBlob struct {
	api.ManifestBlobServiceServer
	store  bare.Store
	w      *watch.Watch
	namer  slug.Namer
	joined bool
...
func (s Sink) ManifestBlob() api.ManifestBlobServiceServer {
	return sinkManifestBlob{s.Server.ManifestBlob(), s.Server.Store, s.w, s.namer, s.joined}
}

// orderManifestBlob is how ManifestBlobs come back.
//
// The last column is the key, and it is not decoration: a cursor cannot
// tell apart two rows equal in every column of the order, so the page after
// the first of them either repeats the second or skips it. Rows written by
// one request are stamped a moment apart at best.
var orderManifestBlob = []sqlpage.Order{
	{Column: manifestblob.FieldDateCreated, Desc: false},
	{Column: manifestblob.FieldId, Desc: false},
}

const (
	// ManifestBlobPageSize is what a request that did not say gets, and
	// ManifestBlobPageLimit is the most it gets however loudly it asks.
	ManifestBlobPageSize  = 20
	ManifestBlobPageLimit = 100

	// ManifestBlobFilterLimit is how many filters one request may carry. Each is a
	// predicate in the same query, so it is what says how much of the
	// database a request may ask to read -- and it is refused rather than
	// clamped, because dropping half the filters would answer a question
	// nobody asked.
	ManifestBlobFilterLimit = 32
)

// List answers with the ManifestBlobs that match any of the given filters, or with
// every one there is if the request named none, a page at a time.
func (s sinkManifestBlob) List(ctx context.Context, req *api.ManifestBlobListRequest) (*api.ManifestBlobListResponse, error) {
	q := s.store.Db.ManifestBlob.Query()

	// Through the same narrowing every generated read goes through, and not
	// by asking the scope alone: what narrows a read is the wall today and
	// the wall and something else tomorrow, and a list that reached past it
	// would be the one read that missed the something else.
	if p, err := bare.ManifestBlobNarrow(ctx, s.store.Scope, nil); err != nil {
		return nil, err
	} else if p != nil {
		q.Where(p)
	}
	...
	if fs := req.GetFilters(); len(fs) > 0 {
		if len(fs) > ManifestBlobFilterLimit {
			return nil, status.Errorf(codes.InvalidArgument,
				"filters: %d of them, and %d is the most one list carries", len(fs), ManifestBlobFilterLimit)
		}

		ps := make([]predicate.ManifestBlob, 0, len(fs))
		for i, f := range fs {
			p, err := filterManifestBlob(f)
			if err != nil {
				return nil, status.Errorf(codes.InvalidArgument, "filters[%d]: %s", i, err)
			}
	...
	if v := req.GetAfter(); v != "" {
		...
		p, err := sqlpage.After(orderManifestBlob, []any{at0, at1})
		if err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "after: %s", err)
		}
	...
	// there -- so a full last page answers with no cursor rather than sending
	// the caller back for an empty one.
	size := sqlpage.Size(int(req.GetSize()), ManifestBlobPageSize, ManifestBlobPageLimit)
	us, err := q.Order(manifestblob.ByDateCreated(), manifestblob.ById()).Limit(size + 1).All(ctx)
	if err != nil {
		return nil, err
	}
	...
	items := make([]*api.ManifestBlob, len(us))
	for i, u := range us {
		items[i] = u.Proto()
	}

	res := api.ManifestBlobListResponse_builder{Items: items}.Build()
	if more {
		last := us[len(us)-1]
		next, err := sqlpage.Encode(last.DateCreated, last.Id)
		if err != nil {
...
// filterManifestBlob turns one filter into the predicate that selects what it
// names. Naming nothing is refused, since the request asked for "these" and
// did not say which.
func filterManifestBlob(f *api.ManifestBlobFilter) (predicate.ManifestBlob, error) {
	ps := make([]predicate.ManifestBlob, 0, 1)
	if f.HasRef() {
		p, err := bare.ManifestBlobPick(f.GetRef())
		if err != nil {
			return nil, err
		}
	...
	if f.HasRepo() {
		ps = append(ps, manifestblob.RepoEQ(f.GetRepo()))
	}
	if f.HasManifest() {
		ps = append(ps, manifestblob.ManifestEQ(f.GetManifest()))
	}
	if f.HasBlob() {
		ps = append(ps, manifestblob.BlobEQ(f.GetBlob()))
	}
	if len(ps) == 0 {
		return nil, status.Error(codes.InvalidArgument, "a filter that names nothing")
	}
...
func (s Sink) Repository() api.RepositoryServiceServer {
	return sinkRepository{s.Server.Repository(), s.Server.Store, s.w, s.namer, s.joined}
}

// orderRepository is how Repositorys come back.
//
// The last column is the key, and it is not decoration: a cursor cannot
// tell apart two rows equal in every column of the order, so the page after
// the first of them either repeats the second or skips it. Rows written by
// one request are stamped a moment apart at best.
var orderRepository = []sqlpage.Order{
	{Column: repository.FieldDateCreated, Desc: false},
	{Column: repository.FieldId, Desc: false},
}

const (
	// RepositoryPageSize is what a request that did not say gets, and
	// RepositoryPageLimit is the most it gets however loudly it asks.
	RepositoryPageSize  = 20
	RepositoryPageLimit = 100

	// RepositoryFilterLimit is how many filters one request may carry. Each is a
	// predicate in the same query, so it is what says how much of the
	// database a request may ask to read -- and it is refused rather than
	// clamped, because dropping half the filters would answer a question
	// nobody asked.
	RepositoryFilterLimit = 32
)

// List answers with the Repositorys that match any of the given filters, or with
// every one there is if the request named none, a page at a time.
func (s sinkRepository) List(ctx context.Context, req *api.RepositoryListRequest) (*api.RepositoryListResponse, error) {
	q := s.store.Db.Repository.Query()

	// Through the same narrowing every generated read goes through, and not
	// by asking the scope alone: what narrows a read is the wall today and
	// the wall and something else tomorrow, and a list that reached past it
	// would be the one read that missed the something else.
	if p, err := bare.RepositoryNarrow(ctx, s.store.Scope, nil); err != nil {
		return nil, err
	} else if p != nil {
		q.Where(p)
	}
	...
	if fs := req.GetFilters(); len(fs) > 0 {
		if len(fs) > RepositoryFilterLimit {
			return nil, status.Errorf(codes.InvalidArgument,
				"filters: %d of them, and %d is the most one list carries", len(fs), RepositoryFilterLimit)
		}

		ps := make([]predicate.Repository, 0, len(fs))
		for i, f := range fs {
			p, err := filterRepository(f)
			if err != nil {
				return nil, status.Errorf(codes.InvalidArgument, "filters[%d]: %s", i, err)
			}
	...
	if v := req.GetAfter(); v != "" {
		...
		p, err := sqlpage.After(orderRepository, []any{at0, at1})
		if err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "after: %s", err)
		}
	...
	// there -- so a full last page answers with no cursor rather than sending
	// the caller back for an empty one.
	size := sqlpage.Size(int(req.GetSize()), RepositoryPageSize, RepositoryPageLimit)
	us, err := q.Order(repository.ByDateCreated(), repository.ById()).Limit(size + 1).All(ctx)
	if err != nil {
		return nil, err
	}
	...
	items := make([]*api.Repository, len(us))
	for i, u := range us {
		items[i] = u.Proto()
	}

	res := api.RepositoryListResponse_builder{Items: items}.Build()
	if more {
		last := us[len(us)-1]
		next, err := sqlpage.Encode(last.DateCreated, last.Id)
		if err != nil {
...
// filterRepository turns one filter into the predicate that selects what it
// names. Naming nothing is refused, since the request asked for "these" and
// did not say which.
func filterRepository(f *api.RepositoryFilter) (predicate.Repository, error) {
	ps := make([]predicate.Repository, 0, 1)
	if f.HasRef() {
		p, err := bare.RepositoryPick(f.GetRef())
		if err != nil {
			return nil, err
		}
	...
	if f.HasName() {
		ps = append(ps, repository.NameEQ(f.GetName()))
	}
	if len(ps) == 0 {
		return nil, status.Error(codes.InvalidArgument, "a filter that names nothing")
	}
...
// RepositoryService is the prefix of every Rpc of that service, which is how a
// change is known to be about a Repository. A service is named for the entity it
// is about, so the name carries it.
var RepositoryService = watch.ServiceOf(api.RepositoryService_Get_FullMethodName)

// Watch answers with the Repositorys this caller may see, as they are now and as
// they change.
//
// What is sent is **state and never a delta**, which is what makes a stream
// that missed something still correct: the next item about a row carries the
// whole of it, so a client converges rather than replays. It is also what
// makes the first message safe to duplicate against the ones after it.
func (s sinkRepository) Watch(req *api.RepositoryWatchRequest, out grpc.ServerStreamingServer[api.RepositoryWatchResponse]) error {
	ctx := out.Context()
	...
	case len(fs) == 0:
		return status.Error(codes.InvalidArgument,
			"filters: a watch says which rows it is about; one that says nothing is the whole table, for as long as it is open")
	case len(fs) > RepositoryFilterLimit:
		return status.Errorf(codes.InvalidArgument,
			"filters: %d of them, and %d is the most one watch carries", len(fs), RepositoryFilterLimit)
	}

	// Resolved before anything is subscribed to, so a name that names
	// nothing is an answer rather than a stream that quietly watches none.
	watching, err := s.watchRepositoryKeys(ctx, fs)
	if err != nil {
		return err
	}
	...
	return watch.Stream(ctx, s.w, RepositoryService, snapshot,
		func(ks map[pdid.Id]string, sent watch.Seen) error {
			items := make([]*api.RepositoryWatchItem, 0, len(ks))
			for k, action := range ks {
				u, err := s.watchRead(ctx, watching, k)
				if err != nil {
					return err
				...
				sent[k] = u != nil
				items = append(items, api.RepositoryWatchItem_builder{
					Id:     k.Bytes(),
					Value:  u,
					Action: action,
				}.Build())
			...
			return out.Send(api.RepositoryWatchResponse_builder{Items: items}.Build())
		})
}
...
// would have called -- so what a stream begins with and what a list answers
// cannot disagree, and a client does not have to do both and race them.
func (s sinkRepository) watchNow(
	ctx context.Context, req *api.RepositoryWatchRequest, out grpc.ServerStreamingServer[api.RepositoryWatchResponse],
	sent watch.Seen,
) error {
	after := ""
	for {
		res, err := s.List(ctx, api.RepositoryListRequest_builder{
			Filters: req.GetFilters(),
			After:   after,
		}.Build())
		if err != nil {
		...
		items := make([]*api.RepositoryWatchItem, 0, len(res.GetItems()))
		for _, u := range res.GetItems() {
			k, err := pdid.From(u.GetId())
			if err != nil {
				return err
			...
			// No action: this is not something anybody asked for, it is
			// what is already there.
			items = append(items, api.RepositoryWatchItem_builder{Id: u.GetId(), Value: u}.Build())
		}
		if len(items) > 0 {
			if err := out.Send(api.RepositoryWatchResponse_builder{Items: items}.Build()); err != nil {
				return err
			}
		}
...
// server every other read does, with the context of the caller who asked, so
// a row they may not see comes back NotFound and is never sent.
func (s sinkRepository) watchRead(
	ctx context.Context, watching []pdid.Id, k pdid.Id,
	...
	v, err := s.Get(ctx, api.RepositoryGetRequest_builder{
		Ref: api.RepositoryRef_builder{Id: k.Bytes()}.Build(),
	}.Build())
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return nil, nil
...
// watchRepositoryKeys is the rows a stream is about, resolved once when it opens.
//
// A filter names a row and a row is named several ways -- by identifier, or
// by whatever unique index the schema declared. Resolving them here rather
// than comparing them per event does three things: the comparison afterwards
...
// row renamed while the stream is open goes on being the row that was asked
// for -- which is what somebody watching a thing meant.
func (s sinkRepository) watchRepositoryKeys(
	ctx context.Context, fs []*api.RepositoryFilter,
) ([]pdid.Id, error) {
	ks := make([]pdid.Id, 0, len(fs))
	for i, f := range fs {
		if !f.HasRef() {
		...
		v, err := s.Get(ctx, api.RepositoryGetRequest_builder{Ref: f.GetRef()}.Build())
		if err != nil {
			return nil, err
		}
...
func (s Sink) Tag() api.TagServiceServer {
	return sinkTag{s.Server.Tag(), s.Server.Store, s.w, s.namer, s.joined}
}

// orderTag is how Tags come back.
//
// The last column is the key, and it is not decoration: a cursor cannot
// tell apart two rows equal in every column of the order, so the page after
// the first of them either repeats the second or skips it. Rows written by
// one request are stamped a moment apart at best.
var orderTag = []sqlpage.Order{
	{Column: tag.FieldDateCreated, Desc: false},
	{Column: tag.FieldId, Desc: false},
}

const (
	// TagPageSize is what a request that did not say gets, and
	// TagPageLimit is the most it gets however loudly it asks.
	TagPageSize  = 20
	TagPageLimit = 100

	// TagFilterLimit is how many filters one request may carry. Each is a
	// predicate in the same query, so it is what says how much of the
	// database a request may ask to read -- and it is refused rather than
	// clamped, because dropping half the filters would answer a question
	// nobody asked.
...
// List answers with the Tags that match any of the given filters, or with
// every one there is if the request named none, a page at a time.
func (s sinkTag) List(ctx context.Context, req *api.TagListRequest) (*api.TagListResponse, error) {
	q := s.store.Db.Tag.Query()

	// Through the same narrowing every generated read goes through, and not
	// by asking the scope alone: what narrows a read is the wall today and
	// the wall and something else tomorrow, and a list that reached past it
	// would be the one read that missed the something else.
	if p, err := bare.TagNarrow(ctx, s.store.Scope, nil); err != nil {
		return nil, err
	} else if p != nil {
		q.Where(p)
	}
	...
	if fs := req.GetFilters(); len(fs) > 0 {
		if len(fs) > TagFilterLimit {
			return nil, status.Errorf(codes.InvalidArgument,
				"filters: %d of them, and %d is the most one list carries", len(fs), TagFilterLimit)
		}

		ps := make([]predicate.Tag, 0, len(fs))
		for i, f := range fs {
			p, err := filterTag(f)
			if err != nil {
				return nil, status.Errorf(codes.InvalidArgument, "filters[%d]: %s", i, err)
	...
	if v := req.GetAfter(); v != "" {
		...
		p, err := sqlpage.After(orderTag, []any{at0, at1})
		if err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "after: %s", err)
		}
	...
	// there -- so a full last page answers with no cursor rather than sending
	// the caller back for an empty one.
	size := sqlpage.Size(int(req.GetSize()), TagPageSize, TagPageLimit)
	us, err := q.Order(tag.ByDateCreated(), tag.ById()).Limit(size + 1).All(ctx)
	if err != nil {
		return nil, err
	}
	...
	items := make([]*api.Tag, len(us))
	for i, u := range us {
		items[i] = u.Proto()
	}

	res := api.TagListResponse_builder{Items: items}.Build()
	if more {
		last := us[len(us)-1]
		next, err := sqlpage.Encode(last.DateCreated, last.Id)
		if err != nil {
...
// filterTag turns one filter into the predicate that selects what it
// names. Naming nothing is refused, since the request asked for "these" and
// did not say which.
func filterTag(f *api.TagFilter) (predicate.Tag, error) {
	ps := make([]predicate.Tag, 0, 1)
	if f.HasRef() {
		p, err := bare.TagPick(f.GetRef())
		if err != nil {
			return nil, err
		}
	...
	if f.HasRepo() {
		ps = append(ps, tag.RepoEQ(f.GetRepo()))
	}
	if f.HasDigest() {
		ps = append(ps, tag.DigestEQ(f.GetDigest()))
	}
	if len(ps) == 0 {
		return nil, status.Error(codes.InvalidArgument, "a filter that names nothing")
	}
...
// TagService is the prefix of every Rpc of that service, which is how a
// change is known to be about a Tag. A service is named for the entity it
// is about, so the name carries it.
var TagService = watch.ServiceOf(api.TagService_Get_FullMethodName)

// Watch answers with the Tags this caller may see, as they are now and as
// they change.
//
// What is sent is **state and never a delta**, which is what makes a stream
// that missed something still correct: the next item about a row carries the
// whole of it, so a client converges rather than replays. It is also what
// makes the first message safe to duplicate against the ones after it.
func (s sinkTag) Watch(req *api.TagWatchRequest, out grpc.ServerStreamingServer[api.TagWatchResponse]) error {
	ctx := out.Context()
	...
	case len(fs) == 0:
		return status.Error(codes.InvalidArgument,
			"filters: a watch says which rows it is about; one that says nothing is the whole table, for as long as it is open")
	case len(fs) > TagFilterLimit:
		return status.Errorf(codes.InvalidArgument,
			"filters: %d of them, and %d is the most one watch carries", len(fs), TagFilterLimit)
	}

	// Resolved before anything is subscribed to, so a name that names
	// nothing is an answer rather than a stream that quietly watches none.
	watching, err := s.watchTagKeys(ctx, fs)
	if err != nil {
		return err
	}
	...
	return watch.Stream(ctx, s.w, TagService, snapshot,
		func(ks map[pdid.Id]string, sent watch.Seen) error {
			items := make([]*api.TagWatchItem, 0, len(ks))
			for k, action := range ks {
				u, err := s.watchRead(ctx, watching, k)
				if err != nil {
					return err
				...
				sent[k] = u != nil
				items = append(items, api.TagWatchItem_builder{
					Id:     k.Bytes(),
					Value:  u,
					Action: action,
				}.Build())
			...
			return out.Send(api.TagWatchResponse_builder{Items: items}.Build())
		})
}
...
// would have called -- so what a stream begins with and what a list answers
// cannot disagree, and a client does not have to do both and race them.
func (s sinkTag) watchNow(
	ctx context.Context, req *api.TagWatchRequest, out grpc.ServerStreamingServer[api.TagWatchResponse],
	sent watch.Seen,
) error {
	after := ""
	for {
		res, err := s.List(ctx, api.TagListRequest_builder{
			Filters: req.GetFilters(),
			After:   after,
		}.Build())
		if err != nil {
		...
		items := make([]*api.TagWatchItem, 0, len(res.GetItems()))
		for _, u := range res.GetItems() {
			k, err := pdid.From(u.GetId())
			if err != nil {
				return err
			...
			// No action: this is not something anybody asked for, it is
			// what is already there.
			items = append(items, api.TagWatchItem_builder{Id: u.GetId(), Value: u}.Build())
		}
		if len(items) > 0 {
			if err := out.Send(api.TagWatchResponse_builder{Items: items}.Build()); err != nil {
				return err
			}
		}
...
func (s sinkTag) watchRead(
	...
	v, err := s.Get(ctx, api.TagGetRequest_builder{
		Ref: api.TagRef_builder{Id: k.Bytes()}.Build(),
	}.Build())
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return nil, nil
...
// watchTagKeys is the rows a stream is about, resolved once when it opens.
//
// A filter names a row and a row is named several ways -- by identifier, or
// by whatever unique index the schema declared. Resolving them here rather
// than comparing them per event does three things: the comparison afterwards
...
// row renamed while the stream is open goes on being the row that was asked
// for -- which is what somebody watching a thing meant.
func (s sinkTag) watchTagKeys(
	ctx context.Context, fs []*api.TagFilter,
) ([]pdid.Id, error) {
	ks := make([]pdid.Id, 0, len(fs))
	for i, f := range fs {
		if !f.HasRef() {
		...
		v, err := s.Get(ctx, api.TagGetRequest_builder{Ref: f.GetRef()}.Build())
		if err != nil {
			return nil, err
		}
...
type interceptManifestBlob struct {
	Intercept
	api.ManifestBlobServiceServer
}

func (s interceptManifestBlob) Get(ctx context.Context, req *api.ManifestBlobGetRequest) (*api.ManifestBlob, error) {
	return grpcx.RunUnary(ctx, s.unary, s.ManifestBlobServiceServer,
		api.ManifestBlobService_Get_FullMethodName, req, s.ManifestBlobServiceServer.Get)
}

func (s interceptManifestBlob) List(ctx context.Context, req *api.ManifestBlobListRequest) (*api.ManifestBlobListResponse, error) {
	return grpcx.RunUnary(ctx, s.unary, s.ManifestBlobServiceServer,
		api.ManifestBlobService_List_FullMethodName, req, s.ManifestBlobServiceServer.List)
}

func (s Intercept) Manifest() api.ManifestServiceServer {
	return interceptManifest{s, s.Next().Manifest()}
}
...
func (s interceptManifest) Get(ctx context.Context, req *api.ManifestGetRequest) (*api.Manifest, error) {
	return grpcx.RunUnary(ctx, s.unary, s.ManifestServiceServer,
		api.ManifestService_Get_FullMethodName, req, s.ManifestServiceServer.Get)
}

func (s interceptManifest) List(ctx context.Context, req *api.ManifestListRequest) (*api.ManifestListResponse, error) {
	return grpcx.RunUnary(ctx, s.unary, s.ManifestServiceServer,
		api.ManifestService_List_FullMethodName, req, s.ManifestServiceServer.List)
}

func (s interceptManifest) Watch(req *api.ManifestWatchRequest, out grpc.ServerStreamingServer[api.ManifestWatchResponse]) error {
	return grpcx.RunStream(s.stream, s.ManifestServiceServer,
		api.ManifestService_Watch_FullMethodName, req, out, s.ManifestServiceServer.Watch)
}
...
func (s interceptRepository) Get(ctx context.Context, req *api.RepositoryGetRequest) (*api.Repository, error) {
	return grpcx.RunUnary(ctx, s.unary, s.RepositoryServiceServer,
		api.RepositoryService_Get_FullMethodName, req, s.RepositoryServiceServer.Get)
}

func (s interceptRepository) Patch(ctx context.Context, req *api.RepositoryPatchRequest) (*api.Repository, error) {
	return grpcx.RunUnary(ctx, s.unary, s.RepositoryServiceServer,
		api.RepositoryService_Patch_FullMethodName, req, s.RepositoryServiceServer.Patch)
}

func (s interceptRepository) Apply(ctx context.Context, req *api.RepositoryApplyRequest) (*api.Repository, error) {
	return grpcx.RunUnary(ctx, s.unary, s.RepositoryServiceServer,
		api.RepositoryService_Apply_FullMethodName, req, s.RepositoryServiceServer.Apply)
}

func (s interceptRepository) List(ctx context.Context, req *api.RepositoryListRequest) (*api.RepositoryListResponse, error) {
	return grpcx.RunUnary(ctx, s.unary, s.RepositoryServiceServer,
		api.RepositoryService_List_FullMethodName, req, s.RepositoryServiceServer.List)
}
...
func (s Intercept) Tag() api.TagServiceServer {
	return interceptTag{s, s.Next().Tag()}
}

type interceptTag struct {
	Intercept
	api.TagServiceServer
}

func (s interceptTag) Get(ctx context.Context, req *api.TagGetRequest) (*api.Tag, error) {
	return grpcx.RunUnary(ctx, s.unary, s.TagServiceServer,
		api.TagService_Get_FullMethodName, req, s.TagServiceServer.Get)
}

func (s interceptTag) List(ctx context.Context, req *api.TagListRequest) (*api.TagListResponse, error) {
	return grpcx.RunUnary(ctx, s.unary, s.TagServiceServer,
		api.TagService_List_FullMethodName, req, s.TagServiceServer.List)
}

func (s interceptTag) Watch(req *api.TagWatchRequest, out grpc.ServerStreamingServer[api.TagWatchResponse]) error {
	return grpcx.RunStream(s.stream, s.TagServiceServer,
		api.TagService_Watch_FullMethodName, req, out, s.TagServiceServer.Watch)
}
...
func dispatch(ctx context.Context, s api.Server, op *pdpb.Op) (*anypb.Any, error) {
	...
	case api.ManifestBlobService_Get_FullMethodName:
		...
		res, err := s.ManifestBlob().Get(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.ManifestBlobService_List_FullMethodName:
		v := &api.ManifestBlobListRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.ManifestBlob().List(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.ManifestService_Get_FullMethodName:
		v := &api.ManifestGetRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Manifest().Get(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.ManifestService_List_FullMethodName:
		v := &api.ManifestListRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Manifest().List(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.RepositoryService_Get_FullMethodName:
		v := &api.RepositoryGetRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Repository().Get(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.RepositoryService_Patch_FullMethodName:
		v := &api.RepositoryPatchRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Repository().Patch(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.RepositoryService_Apply_FullMethodName:
		v := &api.RepositoryApplyRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Repository().Apply(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.RepositoryService_List_FullMethodName:
		v := &api.RepositoryListRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}
	...
	case api.TagService_Get_FullMethodName:
		v := &api.TagGetRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Tag().Get(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case api.TagService_List_FullMethodName:
		v := &api.TagListRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Tag().List(ctx, v)
		if err != nil {
			return nil, err
		}
